...
from ..access_control import FULL_ACCESS_ROLES
from ..auth_decorator import token_required
from ..responses import error_response, success_response
from ..services import contas_bordo_service
from ..services import gestao_service


@csrf_exempt
@token_required(roles=FULL_ACCESS_ROLES)
...
@token_required(roles=FULL_ACCESS_ROLES)
@require_http_methods(["GET"])
def get_contas_em_bordo(request):
    try:
        return success_response(contas_bordo_service.buscar_contas_em_bordo())
    except Exception as exc:
        return error_response(str(exc), 'SERVER_ERROR', 500)
